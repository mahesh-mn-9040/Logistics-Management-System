...
$orderSql = "SELECT * FROM orders WHERE order_status = 1 AND companyid='$companyid'";
...
$userwisesql = "SELECT users.username , SUM(orders.grand_total) as totalorder FROM orders INNER JOIN users ON orders.user_id = users.user_id  WHERE orders.order_status = 1 AND users.companyid='$companyid'  GROUP BY orders.user_id ";
